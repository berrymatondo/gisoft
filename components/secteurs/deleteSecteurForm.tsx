...
import { deleteSecteur, updateSecteur } from "@/app/_actionsSecteur";
...
const DeleteSecteurForm = ({ secteur }: EditSecteurProps) => {
  ...
  const procesForm = async (values: z.infer<typeof secteurFormSchema>) => {
    ...
    const res = await deleteSecteur(values);
    ...
    toast.success("Secteur a été supprimé avec succes.", {
    ...
    <div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-[425px] bg-[#1b4c48] text-white">
          <DialogHeader>
            <DialogTitle>Supprimer un secteur</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(procesForm)}>
              <div className="grid gap-4 py-4">
                <Label className="text-center">
                  {`Etes-vous sûr de vouloir supprimer le secteur `}{" "}
                  <strong className="text-yellow-400">{secteur.name}</strong>{" "}
                  {`?`}
                </Label>
                {/*                 <FormField
...
export default DeleteSecteurForm;
